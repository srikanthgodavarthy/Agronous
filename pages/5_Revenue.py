...
c1.metric("Total Revenue", format_currency(total))
c2.metric("Sale Entries", len(revenues_data))
total_qty = sum(r["quantity"] for r in revenues_data)
c3.metric("Total Quantity Sold", f"{total_qty:.1f}" if revenues_data else "—")
    ...
    if revenues_data:
        for row in revenues_data:
            with st.container(border=True):
                ...
                c2.markdown(f"**{row['buyer'] or 'Unknown Buyer'}**")
                c3.markdown(f"{row['quantity']:.1f} {row['quantity_unit']}")
                c4.markdown(f"@ {format_currency(row['price_per_unit'])}")
                c5.markdown(f"**{format_currency(row['amount'])}**")
                if st.button("🗑️ Delete", key=f"del_rev_{row['id']}"):
                    with session_scope() as session:
        ...
        st.info("No sales logged yet. Add your first one on the right.")

with right:
    st.subheader("➕ Add Sale")
    with st.form("add_revenue_form", clear_on_submit=True):
        sale_date = st.date_input("Date *", value=date.today())
        buyer = st.text_input("Buyer", placeholder="e.g. Local Mandi / Trader Name")
        c1, c2 = st.columns(2)
        quantity = c1.number_input("Quantity *", min_value=0.0, step=1.0, value=0.0)
        quantity_unit = c2.selectbox("Unit", ["Quintal", "Kg", "Tonne", "Bag"])
        price_per_unit = st.number_input("Price per Unit *", min_value=0.0, step=10.0, value=0.0)

        if quantity > 0 and price_per_unit > 0:
            st.caption(f"Amount: {format_currency(quantity * price_per_unit)}")
        ...
        if submitted:
            if quantity <= 0 or price_per_unit <= 0:
                ...
                st.success(f"Logged sale of {format_currency(quantity * price_per_unit)}.")
                st.rerun()
