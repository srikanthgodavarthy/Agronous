...
.act-status-row {
    ...
    min-height: 56px; /* fixed regardless of whether a pill is inside —
                          Completed/Skipped render this row empty, and
                          without a fixed height the card would shrink
                          by exactly the pill's height whenever it's gone */
}
.act-status {
        ...
        for week_num, group in groupby(sorted_acts, key=lambda a: _week_num(a["activity_date"], sowing_date)):
            ...
            for row_start in range(0, len(items), ROW_SIZE):
                ...
                for col, row in zip(cols, row_items):
                    ...
                    # visually stand out as "needs action." The row div
                    # itself is always rendered (fixed-height via CSS) so
                    # removing the pill never shrinks the card.
                    if eff_status in ("PENDING", "OVERDUE"):
                        pill_html = "<div class='act-status status-{}'>{}</div>".format(eff_status, status_label)
                    else:
                        pill_html = ""
